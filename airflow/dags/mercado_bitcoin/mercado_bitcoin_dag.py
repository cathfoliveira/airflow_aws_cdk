...
# Se eu quiser importar as opções de Variables do airflow poderia usar da seguinte maneira:
# config = Variable.get('nome-da-dag',deserialize_json=true)

config = {
    "bucket": "s3-bck-develop-data-lake-raw",
...
default_args = {
    ...
    "depends_on_past": False,  # Se a execucao passada falhar, executa a proxima?
    "provide_context": True,  # Necessario para passar xcom de uma dag pra outra
}

# roda todo dia a meia noite, catchup=se ficar uma semana sem executar, vai continuar desde a ultima execucao de sucesso até a data atual.
dag = DAG(
    "mercado_bitcoin_dag",
